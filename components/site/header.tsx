  ...
  return (
    <header className="border-border bg-card/80 sticky top-0 z-50 border-b backdrop-blur">
      <div className="container flex h-16 items-center justify-between">
        ...
        {/* Mobile toggler */}
        <button
          className="text-muted-foreground hover:bg-muted/30 inline-flex items-center justify-center rounded-md p-2 md:hidden"
          aria-label="Menu"
          onClick={() => setOpen((s) => !s)}
        >
      ...
      {open && (
        <div className="border-border bg-card border-t md:hidden">
          <div className="container flex flex-col gap-1 py-3">
            ...
            <NavLink href="/intro" label="Présentation du quiz" onClick={() => setOpen(false)} />
            <NavLink href="/quiz" label="Questionnaire" onClick={() => setOpen(false)} />
